...
do
    [[ "${line}" == *"Execution of Pre/Post Installation Tasks Failed"* ]] \
        ...
        && ((pkill -9 -f "extracted/tps/lnx64/jre9.0.4/bin/java") || true)
    echo ${line}
done
rm -rf extracted
...
patchelf --set-interpreter "$(cat $NIX_CC/nix-support/dynamic-linker)" $out/opt/SDK/2019.1/tps/lnx64/jre9.0.4/bin/java
echo "ELFs patched"
